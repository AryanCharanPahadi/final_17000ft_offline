...
class _ErrorLogsScreenState extends State<ErrorLogsScreen> {
  ...
  Future<void> _fetchErrorLogs() async {
    ...
    try {
      print("Fetching error logs from: $url"); // Log the URL
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody.startsWith('<!DOCTYPE html>') ||
          ...
          print("API response contains HTML instead of logs."); // Log this case
          ...
          print("API returned an empty response."); // Log empty response
        } else {
          setState(() {
            _errorLogs = responseBody;
            _isLoading = false;
          });
          print("API logs fetched successfully."); // Log success
        }
      } else {
        setState(() {
          _errorMessage =
        ...
        print(
            "Failed to fetch logs. Error code: ${response.statusCode}"); // Log error
      }
    } catch (e) {
      setState(() {
      ...
      print("Error occurred while fetching logs: $e"); // Log exception
    }
  }
